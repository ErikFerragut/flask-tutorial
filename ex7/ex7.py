...
# To make this work, we had to change animal.html to use the lists and the fields.
# Since the number of items in the list can vary, the template needs to do its own
# loops. These changes are all in animal.html; the animal view remains the same.
...
# To Do:
# 1. Read the new animal.html, comparing to the one from ex6 if needed.
#    Notice the loop structure (for fact in fact_list) and the field elements
#    (img_link.src and img_link.alt). Add a new field to each animal in the
#    animal_data dictionary and update animal.html so that those fields will show
#    up in the rendered web page.
# 2. There are a lot more templating tricks that you can use. These are pretty well
#    described at http://jinja.pocoo.org/docs/dev/templates/. Find something on
#    that page to try. (The if command may be a good one to start with. Consider
#    having a condition that adds a line if the title matches some string.)
# 3. Inheritances can chain. Create a new file called nofact.html
#    that extends animal.html and all it does is redefine the
#    fact_list block to be empty. Then create a view for it of the
#    form /nofact/<animal_name>. Does it render pages where the list
#    of animal facts is now missing?
